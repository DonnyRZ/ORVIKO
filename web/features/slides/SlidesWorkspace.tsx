...
export function SlidesWorkspace() {
  ...
  const routeSlideId = searchParams.get('slide_id') ?? ''
  ...
  const loadCurrentSlide = async () => {
    ...
    const data = (await response.json()) as { slide?: Slide; slides?: Slide[] }
    const nextSlide = routeSlideId ? data.slide : Array.isArray(data.slides) ? data.slides[0] : null
    if (!nextSlide) {
